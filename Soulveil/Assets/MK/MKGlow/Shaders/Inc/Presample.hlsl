	...
	HEADER
	{
		...
		half4 source = Presample(PASS_TEXTURE_2D(_SourceTex, sampler_linear_clamp_SourceTex), BLOOM_UV, SOURCE_TEXEL_SIZE); //Bloom is always presampled
			...
			half4 lensFlare = 0;
				...
				float2 ghostsEyeShift = lerp(float2(_EyeShiftAmount, 0), float2(-_EyeShiftAmount, 0), unity_StereoEyeIndex);
			#else
				float2 ghostsEyeShift = float2(0, 0);
			#endif

			float2 lensFlareEyeShiftUV = LENS_FLARE_UV + ghostsEyeShift;
			float2 lensFlareEyeShiftFoveatedHalfUV = foveatedHalfUV;

			[unroll(5)]
			for (int i = 1; i <= LENS_FLARE_GHOST_COUNT; i++)
			{ 
				float2 offset = frac(lensFlareEyeShiftUV + (lensFlareEyeShiftFoveatedHalfUV - lensFlareEyeShiftUV) * LENS_FLARE_GHOST_DISPERSAL * i);

				half weight = pow(1.0 - length(lensFlareEyeShiftFoveatedHalfUV - offset) / length(lensFlareEyeShiftFoveatedHalfUV), LENS_FLARE_GHOST_FADE);
			...
			#ifdef UNITY_SINGLE_PASS_STEREO
				lensFlare *= SampleTex2DNoScale(PASS_TEXTURE_2D(_LensFlareColorRamp, sampler_linear_clamp_LensFlareColorRamp), abs(length(UV_HALF - lensFlareEyeShiftUV - STEREO_OFFSET)) / length(UV_HALF));
			#else
				lensFlare *= SampleTex2DNoScale(PASS_TEXTURE_2D(_LensFlareColorRamp, sampler_linear_clamp_LensFlareColorRamp), abs(length(UV_HALF - lensFlareEyeShiftUV)) / length(UV_HALF));
			#endif
			#ifdef COLORSPACE_GAMMA
