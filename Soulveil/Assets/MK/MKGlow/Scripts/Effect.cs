...
namespace MK.Glow
{
    ...
    internal sealed partial class Effect
    #else
    ...
    #endif
    {
        ...
        private void UpdateRenderBuffers()
        {
            ...
            Quality quality = _settings.GetQuality();
            float qualityLevel = (float) quality;
            ...
            renderDimension = new RenderDimension(Mathf.CeilToInt(_sourceContext[0].width / (qualityLevel - anamorphic.x)), Mathf.CeilToInt(_sourceContext[0].height / (qualityLevel - anamorphic.y)));
        ...
        {      
            ...
            if(_renderingCamera.cameraType == CameraType.Game)
                SetVector(PipelineProperties.ShaderProperties.renderTargetSize, new Vector4(_cameraData.GetCameraWidthRaw(), _cameraData.GetCameraHeightRaw(), _cameraData.GetCameraWidthRaw(), _cameraData.GetCameraHeightRaw()), true);
            else
                SetVector(PipelineProperties.ShaderProperties.renderTargetSize, new Vector4(_cameraData.GetCameraWidth(), _cameraData.GetCameraHeight(), _cameraData.GetCameraWidthRaw(), _cameraData.GetCameraHeightRaw()), true);
            ...
            if(_useLensSurface)
            {
                ...
                float dirtXRatio = _cameraData.GetStereoEnabled() ? 2f : 1f;
            ...
            if(_useLensFlare)
            {
                ...
                var leftView = _renderingCamera.GetStereoViewMatrix(Camera.StereoscopicEye.Left);
                var rightView = _renderingCamera.GetStereoViewMatrix(Camera.StereoscopicEye.Right);

                Vector3 leftPos = leftView.inverse.GetColumn(3);
                Vector3 rightPos = rightView.inverse.GetColumn(3);
                float ipd = Vector3.Distance(leftPos, rightPos);

                SetFloat(PipelineProperties.ShaderProperties.eyeShiftAmount, ipd * 0.5f, true);
            }
        ...
        private void Composite()
        {
            ...
            if(_finalBlit)
            {
                ...
                if(_renderingCamera.cameraType == CameraType.Game)
                    Draw(new RenderDimension(_cameraData.GetCameraWidthRaw(), _cameraData.GetCameraHeightRaw()), true);
                else
                    Draw(new RenderDimension(_cameraData.GetCameraWidth(), _cameraData.GetCameraHeight()), true);
