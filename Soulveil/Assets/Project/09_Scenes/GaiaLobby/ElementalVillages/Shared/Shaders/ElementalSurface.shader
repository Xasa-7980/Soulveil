Shader "Elemental Villages/Soft Anime Surface"
{
 Properties
 {
  ...
  _DetailStrength("Texture detail",Range(0,1))=0.3
 ...
 SubShader
 {
  ...
  Pass
  {
   ...
   half4 Frag(Var i):SV_Target
   {
    ...
    half3 albedo=lerp(half3(1,1,1),SAMPLE_TEXTURE2D(_BaseMap,sampler_BaseMap,i.uv).rgb,_DetailStrength)*_BaseColor.rgb;
  ...
  Pass
  {
   ...
   half4 NormalsFrag(Var i):SV_Target
   {
    float3 n=normalize(i.normalWS);
    #if defined(_GBUFFER_NORMALS_OCT)
     float2 oct=PackNormalOctQuadEncode(n);float2 remap=saturate(oct*.5+.5);
     return half4(PackFloat2To888(remap),0);
    #else
     return half4(n,0);
    #endif
   }
