...
namespace JapanesePostFX
{
    ...
    public sealed class JapanesePaintingFeature : ScriptableRendererFeature
    {
        [Tooltip("Assign the supplied JapanesePainting_Post material to preserve the shader in builds.")]
        ...
        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData data)
        {
            ...
            var volume = VolumeManager.instance.stack?.GetComponent<JapanesePaintingVolume>();
        ...
        struct Settings
        {
            public float intensity,bands,whitePoint,saturation,inkStrength,inkWidth,depthThreshold,normalThreshold,normalEdges,colorEdges,paperStrength,paperScale,wobble,pigment,monochrome,washRadius,washStrength,bleed;
            public Color paperColor,inkColor;
            public Settings(JapanesePaintingVolume v)
            {
                ...
                washRadius=v.washRadius.value;
                washStrength=v.washStrength.value;
                bleed=v.bleed.value;
            ...
            public void Apply(Material m)
            {
                ...
                m.SetFloat("_WashRadius",washRadius);
                m.SetFloat("_WashStrength",washStrength);
                m.SetFloat("_Bleed",bleed);
        ...
        sealed class CelPass : ScriptableRenderPass
        {
            ...
            public override void RecordRenderGraph(RenderGraph graph, ContextContainer frameData)
            {
                ...
                var desc=graph.GetTextureDesc(source);desc.name="Japanese Painting Color";desc.clearBuffer=false;desc.msaaSamples=MSAASamples.None;
                var destination=graph.CreateTexture(desc);
                using (var builder=graph.AddRasterRenderPass<PassData>("Japanese Painting + Outlines",out var data))
                {
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData data)
            {
                ...
                RenderingUtils.ReAllocateHandleIfNeeded(ref temporary,desc,FilterMode.Bilinear,TextureWrapMode.Clamp,name:"JapanesePaintingTemporary");
            }
            public override void Execute(ScriptableRenderContext context,ref RenderingData data)
            {
                if (material==null || temporary==null) return;
                var source=data.cameraData.renderer.cameraColorTargetHandle;
                var cmd=CommandBufferPool.Get("Japanese Painting + Outlines");
                try
                {
